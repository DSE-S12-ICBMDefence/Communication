...
def LinkMarginCalc(freq,rxAntennaGain,EbN0mod,CodeGain):
    ...
    outputPower = 5.5                # power in W
    Ta = 790.                       # Antenna noise temperature in K
    L = 100.9                         # Cable length in meters
...
angles = np.linspace(5, 90, 100)
altitude = 600e3

distance = 0.5 * (-2 * 6371000 * np.sin(angles * np.pi / 180) + np.sqrt(4 * (6371000^2) * \
(np.sin(angles * np.pi / 180)**2) - 4 * (6371000**2 - (6371000 + (altitude * 1000))**2)))
